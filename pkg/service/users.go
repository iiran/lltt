...
func QueryUserDisplayname(username string) (displayname string, err error) {
	...
	if rows, err = db.Query("select display_name from users where username = ?", username); err != nil {
...
func QueryUser(username string) (user model.User, err error) {
	...
	if rows, err = db.Query(q, username); err != nil {
		return user, err
	}
	defer rows.Close()
	if !rows.Next() {
...
func CreateUser(user model.User) (resUser model.User, err error) {
	...
	if _, err = db.Exec(q, user.Username, user.Displayname, user.Email, user.Score, user.Weight); err != nil {
